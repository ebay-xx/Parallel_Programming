...
class SealedDES implements Runnable
{
	...
	private static void makeDESKey(byte[] in, byte[] out)  
  {
	...
	SealedDES (long maxkey, int j, int threads, SealedObject sldObj, long runstart )	{
		this.new_maxkey=maxkey;
		this.new_j=j;
		this.new_threads=threads;
		this.new_sldObj=sldObj;
		this.new_runstart = runstart;
 	}
	...
	public void run (){
    	...
    	for ( long i=(new_maxkey/new_threads*(new_j));i<(new_maxkey/new_threads*(new_j+1));i++ )
    	{
    		// Set the key and decipher the object
    		deccipher.setKey (i);
    		String decryptstr = deccipher.decrypt (new_sldObj);
    		...
    		//PrintStream.printf ("Found decrypt key %016x producing message: %s\n", i , decryptstr);
    		...
    		if ( i % 100000 == 0 )
    		{ 
    			long elapsed = System.currentTimeMillis() - new_runstart;
    			System.out.println ( "Thread " + new_j +" Searched key number " + i + " at " + elapsed + " milliseconds.");
    		}
    	}
	}
	...
	public static void main ( String[] args )
	{
		...
		Thread[] threads = new Thread[num_threads];
	         for (int j=0;j<threads.length;j++) {
	            threads[j] = new Thread(new SealedDES(maxkey, j, num_threads, sldObj, runstart));
	            threads[j].start();
	        }
